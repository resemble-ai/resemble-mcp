...
```ts
const callbackUrl = 'https://example.com/webhooks/detect';

const response = await Resemble.v2.detection.create({
  url: 'https://storage.googleapis.com/resemble-ai-docs-public-files/sts-donor-example.wav',
...
const detectionUuid = response.item?.uuid;
...
const response = await Resemble.v2.detection.detectAndGet({
  url: 'https://storage.googleapis.com/resemble-ai-docs-public-files/sts-donor-example.wav',
  callback_url: callbackUrl,
  visualize: true,
  frame_length: 1,
...
const callbackUrl = 'https://example.com/webhooks/detect';
const filePath = path.resolve('path/to/local-audio.wav');
